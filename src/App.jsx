...
function LaserBeam({ laser, onExpire, onHit }) {
  ...
  const from  = useMemo(() => new THREE.Vector3(...laser.from), [laser.from]);
  ...
  const { midPoint, length, quaternion } = useMemo(() => {
    ...
    const midPoint = from.clone().add(dir.clone().multiplyScalar(0.5));
  ...
  const sidewalks = useMemo(() => {
    ...
    ROAD_CENTRES.forEach(r => {
      [-1,1].forEach(side => {
        ...
        strips.push({ x: r+offset, z: 0, w: sidW, d: sidLen });
        strips.push({ x: 0, z: r+offset, w: sidLen, d: sidW });
      });
    });
    return strips;
  }, []);
