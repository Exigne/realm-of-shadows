/**
 * 🛸 SPRINGFIELD UNDER SIEGE (v3 - Traffic & Hiding Update)
...
function useSpringfieldStore() {
  ...
  const [state, setState] = useState({
    ...
    activeCharIdx: 0,
    chars: initChars(),
    powerUps: POWERUP_SPAWNS.map((p, i) => ({ ...p, id: i, active: true })),
    score: 0,
    alienSlowEnd: 0,
    onlinePlayers: {},
    chatMessages: [],
  ...
  const actions = useMemo(() => ({
    ...
    nextChar: () => setState(s => {
      for (let i = 1; i <= CHARACTERS.length; i++) {
        ...
        if (!s.chars[idx].done) {
          const chars = [...s.chars];
          chars[s.activeCharIdx] = { ...chars[s.activeCharIdx], hidden: false }; // Unhide previous
    ...
    hitChar: charIdx => setState(s => {
      ...
      if (!char || char.done || char.hidden) return s; // Can't hit if hidden
    ...
    reset: () => setState(s => ({
      ...
      chars: initChars(), powerUps: POWERUP_SPAWNS.map((p, i) => ({ ...p, id: i, active: true })), alienSlowEnd: 0, quip: null,
    })),
...
function useHumanAnim({ velRef, isNPC, npcMovingRef }) {
  ...
  useFrame((_,delta) => {
    ...
    if(isMoving) walk.current += delta * 15; // Animation speed

    if(body.current) {
      // Natural torso bob and slight sway
    ...
    if(head.current) {
      // Head counter-rotates to stay looking forward
    ...
    // Pendulum swing for arms and legs (reduced amplitude for realism)
...
function SimpsonsRig({ charCfg, velRef, isNPC, npcMovingRef, isHidden }) {
  ...
  if (isHidden) return null; // Make character disappear when hiding
  ...
  return (
    <group position={[b.x, 0, b.z]}>
      ...
      <Html position={[0, b.h+1, 0]} center>
        <div style={{
          background:'rgba(0,0,0,0.75)', color:'#fff', padding:'2px 8px', borderRadius:8, fontSize:11, fontWeight:'bold',
          whiteSpace:'nowrap', pointerEvents:'none', fontFamily:'Arial,sans-serif', border: b.destFor ? '2px solid #FFD90F' : 'none',
        }}>
          {b.destFor ? '⭐ ' : ''}{b.label}
        </div>
      </Html>
...
function TrafficSystem() {
  ...
  // Define simple rectangular paths for cars to loop around
  ...
  useFrame((_, delta) => {
    ...
    carData.forEach((car, i) => {
      ...
      // Move car along its rectangular path
  ...
  return (
    <group>
      {carData.map((car, i) => (
        <group key={car.id} ref={carRefs.current[i]} position={car.pos}>
  ...
  useFrame(() => {
    ...
    if (age >= 1) {
      if (!hasHit.current) {
        ...
        // Cannot hit if hidden inside a building!
...
function AlienSystem() {
  ...
  const handleFire = (fromPos, targetPos, ufoId) => {
    // If character is hidden, aliens shoot randomly instead of targeting
    const targetChar = state.chars[state.activeCharIdx];
    ...
    setLasers(l => [...l.slice(-12), {
      ...
      to:      [finalTarget.x, 0, finalTarget.z],
...
function CameraRig() {
  ...
  useFrame((_,delta) => {
    ...
    // Zoom in slightly if character is hiding inside a building
...
function PlayerController() {
  ...
  useFrame(({clock}, delta) => {
    ...
    // Building/Door Interaction logic
    ...
    // If moving, automatically unhide
    if ((mx !== 0 || mz !== 0) && charState.hidden) {
      actions.unhide();
    }
    ...
    if (!charState.hidden) {
      ...
      // Stop moving completely if hidden
          ...
          <group key={charCfg.id} ref={charGroupRefs.current[idx]} position={charCfg.startPos.toArray()}>
            ...
            {charSt?.hidden && (
               <Html position={[0, 2, 0]} center>
                 <div style={{ background:'rgba(0,0,0,0.8)', color:'white', padding:'4px 10px', borderRadius:10, fontWeight:'bold', border:'2px solid yellow' }}>
                   👀 HIDDEN!
                 </div>
               </Html>
            )}
            ...
            {!charSt?.hidden && (
              <Html position={[0, 2.9, 0]} center occlude>
                <div style={{
                  background: isActive ? charCfg.shirt : 'rgba(0,0,0,0.6)', color: isActive ? '#111' : '#fff',
                  padding: '2px 10px', borderRadius: 12, fontSize: 12, border: `3px solid ${isActive ? '#FFD90F' : '#555'}`,
                  fontWeight: 'bold', pointerEvents: 'none', whiteSpace: 'nowrap', opacity: charSt?.done ? 0.4 : 1,
                }}>
...
// ─── Component Stubs (To save space, rest are same as before) ───────────────
...
function UFOController({ ufoId, startX, startZ, activeCharGroupRef, alienSlowEndRef, scoreRef, onFire }) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    const speedMul = Date.now() < alienSlowEndRef.current ? 0.15 : 1.0;
  ...
  return (
    <group ref={groupRef} position={[startX, 13, startZ]}>
      <mesh castShadow><cylinderGeometry args={[3.2, 3.8, 0.75, 18]} /><meshStandardMaterial color="#33cc44" metalness={0.8} roughness={0.2} /></mesh>
      <mesh position={[0, 0.65, 0]}><sphereGeometry args={[1.6, 16, 8, 0, Math.PI*2, 0, Math.PI/2]} /><meshStandardMaterial color="#99ffaa" transparent opacity={0.7} /></mesh>
      <mesh position={[0,-0.3,0]}><cylinderGeometry args={[1.2, 1.2, 0.15, 12]} /><meshStandardMaterial color="#00ff66" emissive="#00ff66" emissiveIntensity={0.5} /></mesh>
    </group>
  );
}
...
function GameUI() {
  ...
  if (state.phase === 'start') return (
    <div style={ST.overlay}>
      <div style={ST.modal}>
        <h1 style={{ fontFamily:'Impact, Arial Black', fontSize: 38, color: '#FFD90F', textShadow: '3px 3px 0 #ff0000, 5px 5px 0 #111' }}>SPRINGFIELD UNDER SIEGE</h1>
        <p style={{ color: '#555', marginBottom: 20 }}>Help the Simpsons reach their destinations!</p>
        <button style={ST.startBtn} onClick={startGame}>🛸 START GAME</button>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      {state.nearDoor && !state.chars[state.activeCharIdx].hidden && (
         <div style={{ position:'absolute', top:'60%', left:'50%', transform:'translateX(-50%)', background:'rgba(255,255,255,0.9)', padding:'10px 20px', borderRadius:10, border:'3px solid #7B3F00', fontWeight:'bold', fontSize:18 }}>
      ...
      <div style={ST.topLeft}>
        ...
        <div style={{ fontSize:13, color:'#666', marginTop:2 }}>Score: <b>{state.score}</b></div>
      </div>

      <div style={ST.topRight}>
        <div style={{ fontSize:12, fontWeight:'bold', marginBottom:4, color:'#555' }}>SPRINGFIELD FAMILY</div>
        {CHARACTERS.map((c, i) => {
          const cs = state.chars[i];
          const isAct = i === state.activeCharIdx;
          return (
            <div key={c.id} style={{ display:'flex', alignItems:'center', gap:6, padding:'4px 8px', borderRadius:8, marginBottom:3, background: isAct ? '#fffbe6' : 'rgba(255,255,255,0.4)', border: isAct ? '2px solid #FFD90F' : '2px solid transparent', opacity: cs?.done ? 0.4 : 1, cursor: 'pointer' }} onClick={() => actions.switchChar(i)}>
              <span>{c.emoji} {c.name} {cs?.done && '✅'}</span>
            </div>
          );
        })}
        <div style={{ marginTop:4, fontSize:11, color:'#888' }}><kbd style={ST.kbd}>Tab</kbd> switch · <kbd style={ST.kbd}>Q</kbd> ability</div>
      </div>
    </>
  );
}
...
const ST = {
  ...
  startBtn:  { width:'100%', background:'linear-gradient(135deg,#FFD90F,#ff8800)', color:'#111', border:'none', padding:'16px', borderRadius:16, fontSize:18, fontWeight:900, cursor:'pointer', fontFamily:'Impact, Arial Black' },
  topLeft:   { position:'absolute', top:20, left:20, background:'rgba(255,255,255,0.9)', padding:'10px 16px', borderRadius:14, border:'3px solid #FFD90F', zIndex:50 },
  topRight:  { position:'absolute', top:20, right:20, background:'rgba(255,255,255,0.93)', padding:'10px 14px', borderRadius:14, border:'3px solid #FFD90F', zIndex:50, minWidth:240 },
  quip:      { position:'absolute', top:'42%', left:'50%', transform:'translate(-50%,-50%)', background:'rgba(255,220,0,0.95)', color:'#111', padding:'12px 28px', borderRadius:20, fontWeight:'bold', fontSize:18, border:'3px solid #ff8800' },
