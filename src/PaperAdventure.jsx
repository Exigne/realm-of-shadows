import React, { useRef, useMemo, useState, useCallback, Suspense, createContext } from 'react';
import { Canvas, useFrame, useThree } from '@react-three/fiber';
import { Sky, Environment, ContactShadows, SoftShadows, Float, Sparkles, Instance, Instances, Stars } from '@react-three/drei';
...
function Terrain() { 
  ...
  return (
    <mesh ref={meshRef} receiveShadow name="ground">
  ...
  return (
    <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.8, 0]}>
      ...
      <shaderMaterial
        ...
        uniforms={{
          time: { value: 0 },
          color: { value: new THREE.Color(0x40E0D0) }
        }}
        vertexShader={`
          uniform float time;
        ...
        fragmentShader={`
          ...
          void main() {
            float mixStrength = (vElevation + 0.5) * 0.3;
            vec3 finalColor = mix(color, vec3(1.0), mixStrength);
          ...
          <Float key={item.id} speed={3} rotationIntensity={0.2} floatIntensity={0.3}>
            <mesh position={item.position}>
              ...
              <meshStandardMaterial 
                color={item.type === 'apple' ? 0xFF4500 : 0xFF69B4}
                emissive={item.type === 'apple' ? 0xFF2200 : 0xFF1493}
            ...
            <Sparkles 
              ...
              scale={0.5} 
              size={0.2} 
              color={item.type === 'apple' ? 0xFF4500 : 0xFF69B3}
            />
          </Float>
        );
      })}
...
function DayNightCycle({ time }) { 
  ...
  const sunY = Math.max(Math.sin(-sunAngle) * 60, -10); 
...
function CameraController({ state }) { 
  ...
  useFrame((frameState, delta) => { 
    ...
    camera.position.lerp(targetPos, delta * (isRiding ? 2 : 3));
    ...
    <> 
      ...
      <NPC 
        data={{
          ...
          color: 0x87CEFA,
          earType: 'bear',
          home: new THREE.Vector3(-12, 0, -10),
          timeOffset: 0,
          dialogues: [["Nice day for a hoverboard ride!", "I love cruising around the island."]],
          isTalking: false
        }}
      ...
      <NPC 
        data={{
          ...
          color: 0xDDA0DD,
          earType: 'cat',
          home: new THREE.Vector3(15, 0, -5),
          timeOffset: 2,
          dialogues: [["Meow... the island air is lovely.", "Have you collected many things today?"]],
          isTalking: false
        }}
        onInteract={handleNPCInteract}
      />
      <NPC 
        data={{
          ...
          color: 0xFFE4B5,
          earType: 'bunny',
          home: new THREE.Vector3(-5, 0, 15),
          timeOffset: 4,
          dialogues: [["Hop hop! Vroom vroom!", "Click the hoverboard icon to ride!"]],
          isTalking: false
        }}
        onInteract={handleNPCInteract}
      />
...
const UI = ({ state, actions }) => { 
  ...
  React.useEffect(() => { 
    ...
    const handleKeyUp = (e) => { 
      if (e.code === 'ShiftLeft') actions.setRunning(false); 
    };
    ...
    return (
      <div 
        ...
        style={{ 
          ...
          cursor: 'pointer', 
          zIndex: 150, 
          fontFamily: '"Comic Sans MS", "Nunito", cursive' 
        }}
      > 
        <h1 style={{fontSize: '80px', margin: 0}}>🏝️ CANDY ISLAND</h1> 
        <h2>The Ultimate Joyride 🛹✨</h2> 
    ...
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 10 }}>
      ...
      <div style={{ pointerEvents: 'auto', display: 'flex', gap: '15px', position: 'absolute', top: 20, left: 20 }}>
        ...
        <button
          ...
          style={{
            background: state.uiState === 'inventory' ? '#FFB6C1' : 'rgba(255, 255, 255, 0.95)',
            padding: '10px 20px',
            borderRadius: '25px',
        ...
        <button
          ...
          style={{
            background: state.isRiding ? '#00FA9A' : 'rgba(255, 255, 255, 0.95)',
            padding: '10px 20px',
            borderRadius: '25px',
            border: `4px solid ${state.isRiding ? '#2E8B57' : '#87CEEB'}`,
            boxShadow: '0 6px 12px rgba(0,0,0,0.2)',
            cursor: 'pointer',
            transition: 'all 0.2s',
            fontSize: '32px',
        ...
        >
          ...
          >
            <div style={{
              ...
              alignItems: 'center',
              borderBottom: '4px dashed #FFB6C1',
              paddingBottom: '20px',
              marginBottom: '30px'
            }}>
              <h2 style={{
            ...
            <div style={{
              ...
              {[
                ...
                <div 
                  key={i}
                  style={{
                    background: slot?.count > 0 ? 'white' : 'rgba(255,255,255,0.4)',
                    aspectRatio: '1',
                    borderRadius: '20px',
                    border: slot?.count > 0 ? `4px solid ${slot.color}` : '4px dashed #ccc',
                    display: 'flex',
                    flexDirection: 'column',
                    alignItems: 'center',
                    justifyContent: 'center',
      ...
      {state.uiState === 'dialogue' && state.dialogue && (
        <div 
          onClick={() => {
            if (state.dialogue.current < state.dialogue.texts.length - 1) {
              actions.setDialogue({
                ...state.dialogue,
                current: state.dialogue.current + 1
              });
              audio.sfx('talk');
            } else {
        ...
        >
          <div style={{
            ...
            <div style={{
              background: `#${state.dialogue.color.toString(16).padStart(6, '0')}`,
              color: 'white',
              padding: '8px 24px',
            ...
            <div style={{
              ...
              <div style={{
                ...
                ▼ Click to continue ({state.dialogue.current + 1}/{state.dialogue.texts.length})
              </div>
            </div>
          </div>
        </div>
